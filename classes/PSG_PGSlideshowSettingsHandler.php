...
$stylesett[self::$styleSettingsKey]=self::getDefaultStyleSettings(false);
		...
		{
			$PTSETTINGS[self::$settingsKey]=array();
			
				foreach($sett[self::$settingsKey] as $key=>$varr)
			{
			
			if(isset($_POST[self::$settingsKey][$key]))	
			...
			is_array($_POST[self::$slidesKey]))
		{
			...
			if(!isset($PTVAR[self::$slidesKey])) $PTVAR[self::$slidesKey]=array();
			
			//print_r($_POST[self::$slidesKey]);
			//echo "GGGG";
			foreach($_POST[self::$slidesKey] as $index=>$valarr){
			if(!isset($PTVAR[self::$slidesKey][$index])) $PTVAR[self::$slidesKey][$index]=array();
		...
		if(!isset($PTVAR[self::$slidesKey][$index][$key])) $PTVAR[self::$slidesKey][$index][$key]='';
...
default:
if(isset($_POST[self::$slidesKey][$index][$key]))
$PTVAR[self::$slidesKey][$index][$key]=sanitize_text_field($_POST[self::$slidesKey][$index][$key]);
break;
...
static function getDefaultSettings($fullDefinition = false, $fromDatabase = true)
	{
		...
		if ($fullDefinition)
		{
			...
			'infiniteLoop' => 'If yes, clicking "Next" while on the last slide will transition to the first slide and vice-versa',
			'hideControlOnEnd' => 'If yes, "Prev" and "Next" controls will be hidden at start and end.',
			'captions' => 'Include image captions. ',
			'adaptiveHeight' => 'Dynamically adjust slider height based on each slider width',
			'video' => 'If any slides contain video, set this to yes.',
			'responsive' => 'Enable or disable auto resize of the slider. ',
			'preloadImages' => 'Preload only visible or all or none.',
			'touchEnabled' => 'If yes, slider will allow touch swipe transitions',
			'pager' => 'If yes, a pager will be added',
			'pagerType' => 'Short or Full Pager Type',
			'controls' => 'If yes, "Next" / "Prev" controls will be added',
			'auto' => 'Slides will automatically transition if set to yes',
			...
			'shrinkItems' => 'Carousel: Will only show whole items and shrink the images to fit the viewport based on maxSlides / minSlides.',
			'slideWidth' => 'The width of each slide, This setting is required if need horizontal carousels!',
			...
			$data = array(
				...
				'shrinkItems'=> array('type' => 'radio'  , 'default' => $data['shrinkItems']              , 'description' => $descriptions['shrinkItems']              , 'group' => __('Slideshow', 'portfolio-gallery'),'options' => array(true => $yes, false => $no)),
			...
			'sw_velocity'=>array('type' => 'text'  , 'default' => $data['sw_velocity']           , 'description' => $descriptions['sw_velocity']           , 'group' => __('Skitter', 'portfolio-gallery')),
		...
		switch($settings['type'])
		{
			...
			case 'radio':
				...
				foreach ($settings['options'] as $radioKey => $radioValue)
				{
					...
					///echo "ooo".$displayValue."--".$radioKey;
